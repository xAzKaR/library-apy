...
//        Execução (When)
    ...
    public void updateBookTest() throws Exception {
        ...
        Book updatingBook = Book.builder().id(10L).author("Algum titulo").title("Algum autor").isbn("321").build();
        BDDMockito.given(service.getById(anyLong())).willReturn(Optional.of(updatingBook));

        Book bookUpdating = Book.builder().id(id).author("Artur").title("As Aventuras").isbn("321").build();
        BDDMockito.given(service.update(updatingBook)).willReturn(bookUpdating);

        MockHttpServletRequestBuilder request = MockMvcRequestBuilders
                .put(BOOK_API.concat("/" + 10L))
                .content(json)
                .accept(MediaType.APPLICATION_JSON)
                .contentType(MediaType.APPLICATION_JSON);
